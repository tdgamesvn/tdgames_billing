...
1. **[Tải hóa đơn](/vi/einvoice-api/tai-hoa-don)** - Tải file PDF hoặc XML của hóa đơn đã phát hành
2. **[Chi tiết hóa đơn](/vi/einvoice-api/chi-tiet-hoa-don)** - Xem thông tin chi tiết hóa đơn (số hóa đơn, ngày phát hành, URL file...)
3. **[Danh sách hóa đơn](/vi/einvoice-api/danh-sach-hoa-don)** - Quản lý và tra cứu các hóa đơn đã phát hành

Nếu `status` là `Failed`:

* Kiểm tra `message` để biết nguyên nhân lỗi, sửa thông tin và thử **[phát hành lại](/vi/einvoice-api/phat-hanh-hoa-don-dien-tu)** hoặc liên hệ SePay để được hỗ trợ