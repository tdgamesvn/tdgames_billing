...
interface FieldDef {
  key: string; label: string; type: 'text' | 'date' | 'select' | 'email';
  placeholder?: string; required: boolean;
  options?: { value: string; label: string }[];
}

interface Section {
  icon: string; title: string; required: boolean;
  fields: FieldDef[];
}
...
const EMPLOYEE_SECTIONS: Section[] = [
  ...
  {
    icon: '🏦', title: 'Thông tin ngân hàng', required: true,
    fields: [
  ...
  {
    icon: '📄', title: 'Thuế & Bảo hiểm', required: false,
    fields: [
...
export const ProfileCompletionScreen: React.FC<Props> = ({ currentUser, onComplete }) => {
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try { const r = await uploadFileToR2(file); updateField('avatar_url', r.url); }
    catch (err: any) { setError('Upload ảnh thất bại: ' + (err.message || '')); }
  ...
  const lS: React.CSSProperties = { color: '#888', fontSize: '10px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '5px', display: 'block' };
  ...
  const sectionHeaderStyle = (required: boolean): React.CSSProperties => ({
    color: required ? '#06B6D4' : '#666',
    ...
    <div className="min-h-screen relative overflow-hidden" style={{ backgroundColor: '#0F0F0F' }}>
      ...
      <div className="relative z-10 md:h-screen md:flex md:items-center md:justify-center p-4 md:p-6">
        <div className="w-full max-w-[1400px]">
          <div className="rounded-[24px] border p-6 md:p-8" style={{ background: 'rgba(22,22,22,0.9)', backdropFilter: 'blur(24px)', borderColor: 'rgba(6,182,212,0.15)', boxShadow: '0 32px 64px rgba(0,0,0,0.5)' }}>

            {/* ── Top bar ── */}
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 mb-5">
              {/* Branding */}
              <div className="flex items-center gap-3 flex-1">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(6,182,212,0.1)', border: '1px solid rgba(6,182,212,0.3)' }}>
              ...
              <div className="md:w-[200px] flex-shrink-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span style={{ color: pctColor, fontSize: '14px', fontWeight: 900 }}>{pct}%</span>
                </div>
              ...
              {SECTIONS.map((section, idx) => (
                <div key={idx}>
                  ...
                  <div className="flex items-center gap-2 mb-3 pb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    <span style={sectionHeaderStyle(section.required)}>{section.title}</span>
                    {section.required ? (
                      <span className="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: 'rgba(6,182,212,0.1)', color: '#06B6D4' }}>Bắt buộc</span>
                    ) : (
                      <span className="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: 'rgba(255,255,255,0.04)', color: '#555' }}>Không bắt buộc</span>
                    )}
                  </div>
                  {/* Fields grid — 1 col for small sections, dynamic for larger */}
                  <div className={`grid gap-x-4 gap-y-3 ${section.fields.length <= 3 ? 'grid-cols-1 md:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'}`}>
            ...
            {/* Error */}
            {error && (
              <div className="rounded-xl px-4 py-2.5 text-xs font-bold flex items-center gap-2 mb-4"
            ...
            <button onClick={handleSubmit} disabled={saving}
              className="w-full rounded-xl py-3.5 text-sm font-black uppercase tracking-widest transition-all"
              style={{
