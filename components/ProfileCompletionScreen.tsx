...
const REQUIRED_FIELDS = [
  ...
  { key: 'phone', label: 'Số điện thoại', type: 'text', placeholder: '0912 345 678' },
  { key: 'date_of_birth', label: 'Ngày sinh', type: 'date', placeholder: '' },
  { key: 'gender', label: 'Giới tính', type: 'select', options: [
    { value: 'male', label: 'Nam' },
    { value: 'female', label: 'Nữ' },
    { value: 'other', label: 'Khác' },
  ]},
  { key: 'address', label: 'Địa chỉ thường trú', type: 'text', placeholder: 'Số nhà, đường, quận, TP...' },
  { key: 'id_number', label: 'Số CMND/CCCD', type: 'text', placeholder: '012345678901' },
  { key: 'bank_name', label: 'Tên ngân hàng', type: 'text', placeholder: 'VCB, ACB, MB...' },
  { key: 'bank_account', label: 'Số tài khoản', type: 'text', placeholder: '1234567890' },
];
...
export const ProfileCompletionScreen: React.FC<Props> = ({ currentUser, onComplete }) => {
  ...
  useEffect(() => {
    if (!currentUser.employee_id) {
      // No employee_id = can't load profile, skip
      onComplete();
      return;
    }
    setIsLoading(true);
    fetchMyProfile(currentUser.employee_id)
      ...
      .catch(() => {
        // If profile fails to load, skip completion
        onComplete();
      })
      ...
      <div className="relative z-10 w-full max-w-2xl mx-4 my-8 max-h-[90vh] overflow-y-auto" style={{ animation: 'fadeInUp 0.5s ease-out' }}>
        ...
        >
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            >
              Vui lòng điền đầy đủ thông tin để sử dụng hệ thống
            </p>
          </div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: '#9D9C9D' }}>
                Hoàn thiện hồ sơ
              </span>
              ...
              <div
                ...
                style={{
                  ...
                  background: completionPct >= 90
                    ...
                    : completionPct >= 60
                    ? 'linear-gradient(90deg, #FF9500, #F59E0B)'
          ...
          <div className="flex justify-center mb-6">
            ...
            >
              {form.avatar_url ? (
                <div className="w-full h-full rounded-full overflow-hidden border-2" style={{ borderColor: 'rgba(6,182,212,0.4)' }}>
                ...
                <div
                  className="w-full h-full rounded-full flex flex-col items-center justify-center border-2 border-dashed transition-all group-hover:border-cyan-500/50"
                ...
                >
                  {uploadingAvatar ? (
                    ...
                    <>
                      ...
                      <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: '#06B6D4' }}>Ảnh</span>
                    </>
                  )}
                </div>
              )}
              ...
              <div key={field.key} className={field.key === 'address' ? 'md:col-span-2' : ''}>
                ...
                {field.type === 'select' ? (
                  <select
                    ...
                    style={inputStyle}
                    onFocus={e => { e.target.style.borderColor = 'rgba(6,182,212,0.55)'; e.target.style.boxShadow = '0 0 0 4px rgba(6,182,212,0.07)'; }}
                    onBlur={e => { e.target.style.borderColor = 'rgba(6,182,212,0.15)'; e.target.style.boxShadow = 'none'; }}
                  >
          ...
          {error && (
            <div
              className="rounded-xl px-4 py-3 text-xs font-bold flex items-center gap-2 font-montserrat mb-4"
              style={{
          ...
          {/* Footer */}
          <p
            className="text-center text-[9px] font-bold uppercase tracking-[0.2em] mt-6 font-montserrat"
